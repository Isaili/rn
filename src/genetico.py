...
def seleccion_por_torneo(poblacion, fitnesses, k=3):
    ...
    for _ in range(len(poblacion)):
        ...
        if len(indices_positivos) < k:
            indices_torneo = indices_positivos
        else:
            indices_torneo = random.sample(indices_positivos, k)
...
def mutacion_conservadora(individuo, catalogo, area_total, presupuesto_total, intensidad=0.3):
    """Mutación conservadora sin restricción de trabajadores"""
    ...
    if random.random() < intensidad:
        ...
        costo_unitario = (planta_mutada['fertilizante_por_planta'] * planta_mutada['costo_fertilizante_unitario'] +
        ...
        if max_permitido > 0:
            nuevo[gen_mutado] = random.randint(0, max_permitido)
        else:
            nuevo[gen_mutado] = 0
    else:
...
def reparar_individuo_suave(individuo, catalogo, area_total, presupuesto_total):
    """Reparación suave sin restricción de trabajadores"""
    nuevo = individuo.copy()
    max_iteraciones = 50
    iteracion = 0
...
def algoritmo_genetico(catalogo, area_total, presupuesto_total,
    ...
    """Algoritmo genético sin restricción de trabajadores"""
    ...
    poblacion = [reparar_individuo_suave(ind, catalogo, area_total, presupuesto_total) 
        ...
        while len(nueva_poblacion) < tam_poblacion:
            ...
            if len(indices_validos) >= 2:
                ...
                if len(padres_seleccionados) >= 2:
                    ...
                    if nueva_poblacion:
                        nueva_poblacion.append(nueva_poblacion[0].copy())
            else:
                from poblacion import generar_poblacion_inicial
